...
class DQN:
    def __init__(self, mode='train', *, double_dqn: bool = True):
        self.Hid_Size1 = 10
        self.Hid_Size2 = 50
        self.mode = mode  # 'train' 或 'test'
        # True: Double DQN (online 选动作，target 评估)
        # False: Vanilla DQN (target 直接取 max_a Q_target)
    ...
    def replay(self):
        ...
        with torch.no_grad():
            if self.double_dqn:
                ...
                next_q_values_target = self.target_model(next_states)
                next_q = torch.max(next_q_values_target, dim=1)[0]
        ...
        with torch.no_grad():
            if self.double_dqn:
                ...
                next_q_values_target1 = self.target_model1(next_states1)
                next_q1 = torch.max(next_q_values_target1, dim=1)[0]
    ...
    def Instance_Generator(self, M_num=None, E_ave=None, New_insert=None, mode='train'):
        ...
        # 动态作业到达：泊松过程 => 到达间隔服从指数分布，累加得到到达时刻 Ai
    ...
    def main(self, fixed_params=None, verbose=True):
        """主运行函数（策略由 DQN 选择复合规则索引，不再支持固定单一启发式规则基线）。"""
        k = 0
        Total_tard = []
        Total_makespan = []
        Total_uk_ave = []
