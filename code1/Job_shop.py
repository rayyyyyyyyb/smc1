...
class Situation:
    def __init__(
        ...
        self.EL = EL #紧急程度
        # 是否执行预防性维护（needs_pm 分支）；False 时仍保留换刀、故障维修、健康退化等
        self.enable_preventive_maintenance = enable_preventive_maintenance
    ...
    def Features(self): # 计算6个论文口径状态特征
        ...
        作业集合含「初始作业」与「插入作业」（由 Instance_Generator 生成），下标 i=0..J_num-1 统一编号；
        TR_i、TR_ave 对全部作业计算，不分开统计。
        """
        # 1-2) Uave / Ustd
        ...
        # TR_ave：所有作业（初始+插入）平均处理延迟率 = (Σ_i TR_i) / n，n = J_num
    ...
    def scheduling(self,action):
        ...
        if self.enable_preventive_maintenance and pm.needs_pm():
